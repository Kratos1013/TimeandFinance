 ...
 */
public class add_to_income extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_add_to_income, container, false);
        ...
        if ( descript.equals("") || price.equals("")) {
            ...
            if (new_category.getVisibility() == View.VISIBLE && spinner.getVisibility() == View.GONE) {
                if (categoryname.equals("")){
                    ...
                    price = "+ " + price;
                    boolean insert = db.inserttocategory(categoryname, descript, moment, price, FinanceSQLiteHandler.Table_Name_income, "noicon");
                    if (insert) {
                        boolean add = db.addcategorynamewithicon(categoryname, "noicon", FinanceSQLiteHandler.TABLE_CATEGORIES_Income);
                        if (add) {
                ...
                price = "+ " + price;
                boolean insertto = db.inserttocategory(category, descript, moment, price, FinanceSQLiteHandler.Table_Name_income, "noicon");
                if (insertto) {
                    View view = getActivity().getCurrentFocus();
                    if (view != null) {
                        InputMethodManager imm = (InputMethodManager) getActivity().getSystemService(Context.INPUT_METHOD_SERVICE);
